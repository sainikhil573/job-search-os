...
This log tracks meaningful product, architecture, and workflow decisions for Job Search OS. Entries are factual records, not aspirational plans.
...
- Consequences: The frontend is optimized for client-side application workflows first. Server-side rendering can be revisited later if needed.
- Status: Accepted

## 2026-07-08: Use FastAPI for backend

- Decision: Use FastAPI for the backend application.
- Context: The backend needs clear API boundaries, validation, authentication integration, database access, and future AI service orchestration.
...
- Consequences: Backend development uses Python. API schemas and validation can become a stable contract between frontend and backend.
- Status: Accepted
...
- Context: The project needed lightweight, version-controlled records for status, roadmap, architecture, rules, and decisions before application code existed.
...
- Consequences: Documentation updates are part of normal development workflow. As the project grows, markdown may be supplemented by issues, pull requests, or generated docs.
- Status: Accepted
...
- Consequences: Early phases prioritize resume generation, job tracking, dashboard foundations, authentication, and stable data handling before advanced AI features.
- Status: Accepted
...
- Context: Job Search OS is being built as a professional software product. The repository had application foundation work and a profile module, so future work needed consistent standards for branch workflow, pull requests, validation, documentation updates, and release history.
...
- Reasoning: AI-assisted coding sessions need explicit durable operating instructions so each branch preserves context, avoids scope drift, validates changes consistently, and updates project records.
- Consequences: Contributors and AI coding sessions should use the operating guides and QA checklist before marking work complete. Merged feature history should be maintained in `CHANGELOG.md`, and feature-specific behavior should be documented under `docs/features/`.
- Status: Accepted
...
- Context: Sprint 2A and 2B needed a usable Resume Studio foundation before AI optimization, ATS scoring, document export, or multiple resume versions were introduced.
...
- Reasoning: A text blob would make future editing, previewing, AI suggestions, and section-level updates harder. Fully normalized tables and versioning were more structure than the MVP needed. JSON section fields preserved structured content while keeping the first base-resume workflow small.
- Consequences: The API exposes section arrays such as `skills_json`, `experience_json`, `projects_json`, `education_json`, and `certifications_json`. The external API contract should be revisited before AI optimization, versioning, or export work depends on it.
- Status: Accepted
...
- Context: Sprint 3 needed a manual job tracking foundation that could support later AI job matching, job-specific resume generation, recruiter CRM, interview tracking, and analytics without building those advanced workflows yet.
...
- Context: The Dashboard needed real persisted Job Tracker data for active jobs, archived jobs, active-job status counts, and recently updated jobs. The product is still an early local, single-user MVP without pagination, authentication, multi-user ownership, or analytics-scale data.
- Alternatives considered: Add a dedicated Dashboard summary endpoint, introduce backend aggregation services, add broader analytics models, or continue showing static placeholder metrics.
- Reasoning: The existing jobs endpoint already returned the fields needed for the approved Dashboard MVP. Frontend aggregation avoided premature API surface and kept the sprint focused while replacing misleading static Dashboard metrics.
...
## 2026-07-12: Propose docs PROJECT_OVERVIEW, ROADMAP, and DECISIONS as canonical

- Decision: Proposed, pending approval and commit.
- Proposal: Use `docs/PROJECT_OVERVIEW.md`, `docs/ROADMAP.md`, and `docs/DECISIONS.md` as the canonical long-lived project context records while keeping older root filenames as compatibility notices.
- Context: Repository onboarding found overlapping responsibilities across root status, roadmap, decision, architecture, and feature documents. Some older sprint status text was stale after later merges.
- Alternatives considered: Keep all root documents as independent records, delete old root documents, or duplicate the same information in both root and docs paths.
- Reasoning: Canonical docs under `docs/` would keep durable product, roadmap, and decision context together while avoiding unnecessary duplication. Short compatibility notices would preserve existing references without maintaining two divergent sources of truth.
- Consequences if accepted: Future agents should update the canonical docs first and should not edit compatibility notices except when canonical paths change.
- Status: Proposed

## 2026-07-12: Keep product language aligned with actual data

- Decision: Use product labels and documentation claims that match the implemented data model and behavior.
- Context: Sprint 4 replaced unsupported static Dashboard metrics and used "Recently Updated Jobs" rather than "Recent Activity" because the product has no event log or status-history model.
- Alternatives considered: Use aspirational labels that imply future capabilities, or show placeholder metrics for modules that are not implemented.
